import { getRefrigerantData, isValidRefrigerant } from '../refrigeration/refrigerantData';
...
function interpolatePTForPressure(pt, pressure) {
...
export function runCondenserApproachEngine(measurements, context) {
    ...
    if (condP !== null && profile?.refrigerantType) {
        ...
        if (r && r.pt) {
            const satTemp = interpolatePTForPressure(r.pt, condP);
    ...
    if (combined.includes('critical'))
        status = 'critical';
    else if (combined.includes('alert'))
        status = 'alert';
