import { round, formatTemperature } from '../../shared/wshp.types';
// Helpers: expected ranges (fallback 'industry' if no nameplate data)
export function getExpectedDeltaT(profile, mode) {
    ...
    if (mode === 'fan_only')
        return { min: 0, ideal: 0, max: 0, source: 'industry' };
    if (mode === 'cooling')
        return { min: 8, ideal: 12, max: 16, source: 'industry' };
    return { min: 10, ideal: 20, max: 30, source: 'industry' };
}
export function getExpectedCFMPerTon(profile, mode) {
...
export function requiresManufacturerDataDisclaimer(profile, subject) {
    // Simplified: if profile.model present consider manufacturer-specific; otherwise require disclaimer
    return !profile.model;
}
export function getManufacturerDataDisclaimer(subject) {
...
export function validateAirsideMeasurements(measurements) {
    const errors = [];
    const warnings = [];
    if (!measurements)
        return { valid: false, ok: false, errors: ['No measurements provided'] };
    if (measurements.returnAirTemp === undefined)
        errors.push('returnAirTemp is required');
    if (measurements.supplyAirTemp === undefined)
        errors.push('supplyAirTemp is required');
    if (!measurements.mode)
        warnings.push('mode not provided, assuming cooling');
    const deltaT = Math.abs((measurements.supplyAirTemp ?? 0) - (measurements.returnAirTemp ?? 0));
    if (deltaT < 1)
        errors.push('Delta-T is too small (<1°F) - unit may not be running.');
    return { valid: errors.length === 0, ok: errors.length === 0, errors: errors.length ? errors : undefined, warnings };
}
function analyzeDeltaT(deltaT, expectedRange, mode) {
    if (mode === 'fan_only')
        return { status: 'ok' };
    if (mode === 'cooling') {
        if (deltaT > 30)
            return { status: 'critical' };
        if (deltaT > expectedRange.max)
            return { status: 'alert' };
        if (deltaT < expectedRange.min)
            return { status: 'warning' };
        return { status: 'ok' };
    }
    // heating
    if (deltaT > expectedRange.max || deltaT < expectedRange.min)
        return { status: 'warning' };
    return { status: 'ok' };
}
function analyzeAirflow(cfmPerTon, expectedRange, mode, measured) {
    if (!cfmPerTon)
        return { status: 'ok' };
    ...
    if (cfmPerTon < expectedRange.min)
        return { status: 'alert' };
    if (cfmPerTon > 550)
        return { status: 'alert' };
    if (cfmPerTon > expectedRange.max)
        return { status: 'warning' };
    return { status: 'ok' };
}
function analyzeStaticPressure(totalESP, ratedESP) {
    if (totalESP > ratedESP.max * 1.3)
        return { status: 'critical' };
    if (totalESP > ratedESP.max)
        return { status: 'alert' };
    if (totalESP > ratedESP.max * 0.9)
        return { status: 'warning' };
    return { status: 'ok' };
}
export function generateRecommendations(deltaTAnalysis, airflowAnalysis, staticPressureAnalysis, measurements, profile, expectedCFM) {
    ...
    if (measurements.mode === 'cooling' &&
        ...
        recommendations.push({
            ...
            notes: ['Advisory — do not adjust refrigerant charge solely on this recommendation.'],
        });
    }
...
export function runAirsideEngine(measurements, profile) {
    const disclaimers = [];
    ...
    let staticPressureAnalysis;
    if (measurements.externalStatic !== undefined)
        staticPressureAnalysis = analyzeStaticPressure(measurements.externalStatic, profile.airside.externalStaticPressure);
    const statuses = [deltaTAnalysis.status, airflowAnalysis.status];
    if (staticPressureAnalysis)
        statuses.push(staticPressureAnalysis.status);
    ...
    const flags = {
        ...
        deltaTSource: expectedDeltaTResult.source,
        cfmSource: expectedCFMResult.source,
