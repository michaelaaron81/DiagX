import { REFRIGERANT_DATA, getRefrigerantData } from './refrigerantData';
import { CONSTANTS, round, } from '../../shared/wshp.types';
// Interpolate a PT chart (ptData: Array<[tempF, pressurePSIG]>)
export function interpolatePT(pressure, ptData) {
...
function getSaturationTemp(pressure, refrigerantType) {
    const data = getRefrigerantData(refrigerantType) || REFRIGERANT_DATA[refrigerantType];
    if (!data)
        return 0.215 * pressure + 10.5; // fallback
    const temp = interpolatePT(pressure, data.pt);
    if (temp === null)
        return 0.215 * pressure + 10.5;
    return temp;
}
export function validateRefrigerationMeasurements(measurements) {
    const errors = [];
    const warnings = [];
    if (measurements.suctionPressure <= 0)
        errors.push('Suction pressure must be positive');
    if (measurements.dischargePressure <= 0)
        errors.push('Discharge pressure must be positive');
    ...
    if (measurements.suctionTemp < -40 || measurements.suctionTemp > 150)
        errors.push('Suction temperature out of reasonable range');
    if (measurements.liquidTemp < -20 || measurements.liquidTemp > 250)
        errors.push('Liquid temperature out of reasonable range');
    if (measurements.enteringWaterTemp !== undefined && (measurements.enteringWaterTemp < 20 || measurements.enteringWaterTemp > 150))
        warnings.push('Entering water temp outside typical range (20-150F)');
    if (measurements.leavingWaterTemp !== undefined && (measurements.leavingWaterTemp < 20 || measurements.leavingWaterTemp > 150))
        warnings.push('Leaving water temp outside typical range (20-150F)');
    const valid = errors.length === 0;
    return { valid, ok: valid, errors: valid ? undefined : errors, warnings };
}
function analyzeSuperheat(superheat, mode, metering) {
    ...
    if (superheat < idealRange.min)
        return { status: 'alert' };
    if (superheat > 25)
        return { status: 'alert' };
    if (superheat > idealRange.max)
        return { status: 'warning' };
    if (superheat < idealRange.ideal - 2)
        return { status: 'warning' };
    return { status: 'ok' };
}
function analyzeSubcooling(subcooling, enteringWaterTemp) {
    ...
    if (subcooling < 3)
        return { status: 'alert' };
    if (subcooling < idealRange.min)
        return { status: 'warning' };
    if (subcooling > 18)
        return { status: 'alert' };
    if (subcooling > idealRange.max)
        return { status: 'warning' };
    return { status: 'ok' };
}
function analyzeCompressionRatio(ratio) {
    ...
    if (ratio < 2.5)
        return { status: 'critical' };
    if (ratio < idealRange.min)
        return { status: 'alert' };
    if (ratio > 6.5)
        return { status: 'alert' };
    if (ratio > idealRange.max)
        return { status: 'warning' };
    return { status: 'ok' };
}
function analyzeWaterTransfer(waterDeltaT, tons, designGPM) {
    ...
    if (waterDeltaT < expectedDeltaT * 0.5)
        return { status: 'alert' };
    if (waterDeltaT < minAcceptable)
        return { status: 'warning' };
    if (waterDeltaT > expectedDeltaT * 1.5)
        return { status: 'alert' };
...
export function generateRefrigerationRecommendations(result) {
...
function generateRecommendations(superheatAnalysis, subcoolingAnalysis, compressionRatioAnalysis, waterTransferAnalysis, measurements, profile) {
...
export function runRefrigerationEngine(measurements, config) {
    const refrigerant = config.refrigerant;
    const disclaimers = [];
    // Determine refrigerant profile
    let refrigerantProfile = 'unknown';
    ...
    if (config.ptOverride) {
        ...
        if (refrigerantRaw === 'OTHER') {
            ...
            if (sTemp !== null)
                suctionSatTemp = sTemp;
            if (dTemp !== null)
                dischargeSatTemp = dTemp;
        }
        else {
