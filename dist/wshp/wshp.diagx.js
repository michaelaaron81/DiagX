...
export function runWshpDiagx(input) {
    const { profile, measurements } = input;
    const domainResults = [];
    let airResult = undefined;
    let refResult = undefined;
    let recipResult = undefined;
    let scrollResult = undefined;
    let revResult = undefined;
    if (measurements.airside) {
        airResult = runAirsideEngine(measurements.airside, profile);
    ...
    if (profile.waterSide && measurements.refrigeration && (measurements.refrigeration.enteringWaterTemp !== undefined || measurements.refrigeration.leavingWaterTemp !== undefined)) {
        try {
            ...
            const expectedDelta = profile.waterSide?.expectedDeltaT
                ? { ...profile.waterSide.expectedDeltaT, source: 'profile' }
                : undefined;
    ...
    if (measurements.recipCompressor) {
        recipResult = runReciprocatingCompressorEngine(measurements.recipCompressor, profile);
    ...
    if (measurements.refrigeration && (measurements.refrigeration.ambientTemp !== undefined || measurements.refrigeration.condensingPressure !== undefined)) {
        try {
            ...
            const cProfile = { refrigerantType: profile.refrigeration?.refrigerantType };
    ...
    if (measurements.scrollCompressor) {
        scrollResult = runScrollCompressorEngine(measurements.scrollCompressor, profile);
    ...
    if (measurements.reversingValve) {
        revResult = runReversingValveEngine(measurements.reversingValve, profile);
...
function reduceOverallStatus(results) {
    const statuses = results.flatMap(r => r.findings.map(f => f.severity));
