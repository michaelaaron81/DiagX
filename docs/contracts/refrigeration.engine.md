...
The engine delegates all thermodynamic calculations to the Physics Kernel (`src/physics/hvac`):

| Kernel Function | Used For |
|-----------------|----------|
| `computeSuperheat(suctionTemp, suctionSatTemp)` | Suction superheat calculation |
| `computeSubcooling(dischargeSatTemp, liquidTemp)` | Liquid subcooling calculation |
| `computeCompressionRatio(dischargePressure, suctionPressure)` | Pressure ratio calculation |
| `computeWaterDeltaT(leavingWaterTemp, enteringWaterTemp)` | Water-side ΔT calculation |
| `interpolatePT(pressure, ptData)` | Saturation temperature lookup |
| `getWorstStatus(statuses)` | Status aggregation |
