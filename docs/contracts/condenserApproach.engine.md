...
The engine delegates status aggregation to the Physics Kernel (`src/physics/hvac`):

| Kernel Function | Used For |
|-----------------|----------|
| `getWorstStatus(statuses)` | Status aggregation |

*Note: This engine uses a local PT interpolation implementation with "closest value" fallback behavior that differs from the kernel's extrapolation-based `interpolatePT`. The subcooling calculation also uses a local convention (`liquidTemp - satTemp`) rather than the kernel's standard formula.*
