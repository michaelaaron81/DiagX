...
The engine delegates all thermodynamic calculations to the Physics Kernel (`src/physics/hvac` and `src/physics/hydronic`):

| Kernel Function | Used For |
|-----------------|----------|
| `computeWaterDeltaTAbsolute(leavingWaterTemp, enteringWaterTemp)` | Water temperature differential |
| `computeNormalizedFlowIndex(measuredFlow, designFlow)` | Flow ratio calculation |
