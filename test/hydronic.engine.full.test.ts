...
describe('hydronic engine - full tests', () => {
  it('normal deltaT & flow should be ok', () => {
    const measurements: any = { enteringWaterTemp: 60, leavingWaterTemp: 72, flowRateGPM: 50 };
    const profile: any = { designFlowGPM: 50, expectedDeltaT: { min: 10, ideal: 12, max: 14, source: 'profile' } };
  ...
  it('very low deltaT should be critical', () => {
    const measurements: any = { enteringWaterTemp: 60, leavingWaterTemp: 61, flowRateGPM: 50 };
    const profile: any = { designFlowGPM: 50, expectedDeltaT: { min: 10, ideal: 12, max: 14, source: 'profile' } };
    const res = runHydronicEngine(measurements, { profile });
  ...
  it('missing measurements yield unknown flags', () => {
    const measurements: any = { enteringWaterTemp: null, leavingWaterTemp: null, flowRateGPM: null };
    const profile: any = { designFlowGPM: 50 };
    const res = runHydronicEngine(measurements, { profile });
