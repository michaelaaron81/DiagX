...
describe('airside recommendation helper', () => {
  it('generates critical recommendation for critical deltaT', () => {
    const deltaTAnalysis = { status: 'critical' } as unknown as Record<string, unknown>;
    const airflowAnalysis = { status: 'ok' } as unknown as Record<string, unknown>;
    const recs = generateRecommendations(
      deltaTAnalysis,
      airflowAnalysis,
      undefined,
      { supplyAirTemp: 40, returnAirTemp: 80, mode: 'cooling' } as unknown as Record<string, unknown>,
      { airside: { designCFM: { cooling: 400 }, externalStaticPressure: { design: 0.3, max: 0.6 } } } as unknown as Record<string, unknown>,
      { min: 8, ideal: 12, max: 16 } as unknown as Record<string, unknown>,
    );
    for (const r of recs) {
  ...
  it('returns a preventive low-priority recommendation when conditions are normal', () => {
    const deltaTAnalysis = { status: 'ok' } as unknown as Record<string, unknown>;
    const airflowAnalysis = { status: 'ok' } as unknown as Record<string, unknown>;
    const recs = generateRecommendations(
      deltaTAnalysis,
      airflowAnalysis,
      undefined,
      { supplyAirTemp: 55, returnAirTemp: 70, mode: 'cooling' } as unknown as Record<string, unknown>,
      { airside: { designCFM: { cooling: 400 }, externalStaticPressure: { design: 0.3, max: 0.6 } }, nominalTons: 10 } as unknown as Record<string, unknown>,
      { min: 8, ideal: 12, max: 16 } as unknown as Record<string, unknown>,
    );
