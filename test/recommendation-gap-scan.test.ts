...
for (const s of airScenarios) {
  const r = runAirsideEngine(s.m as unknown as Record<string, unknown>, airProfile as unknown as Record<string, unknown>);
  const gaps:string[] = [];
  ...
  if (r.flags.airflowStatus && r.flags.airflowStatus !== 'ok') {
    if (!hasRelevantRecommendation('airside', r, ['airflow', 'cfm', 'register', 'blower'])) gaps.push('No recommendation relating to airflow (measure/inspect/repair)');
  }
...
const refProfile = { refrigerant: 'R-410A', nominalTons: 10, metering: {cooling:{type:'fixed'}, heating:{type:'fixed'}}, designWaterFlowGPM: 100 } as unknown as Record<string, unknown>;
const refScenarios = [
...
for (const s of refScenarios) {
  const r = runRefrigerationEngine(s.m as unknown as Record<string, unknown>, refProfile as unknown as Record<string, unknown>);
  const gaps:string[] = [];
  ...
  if (r.flags.compressionRatioStatus && r.flags.compressionRatioStatus !== 'ok') {
    if (!hasRelevantRecommendation('refrigeration', r, ['compression', 'bypass', 'reversing'])) gaps.push('No recommendation addressing compression ratio issues');
  }
  ...
  const hydroProfile = { designFlowGPM: 50 } as unknown as Record<string, unknown>;
  ...
  for (const s of hydroScenarios) {
    const r = runHydronicEngine(s.m as unknown as Record<string, unknown>, { profile: hydroProfile } as unknown as Record<string, unknown>);
    const gaps:string[] = [];
  ...
  const condenserProfile = { refrigerantType: 'R-410A' } as unknown as Record<string, unknown>;
  ...
  for (const s of condenserScenarios) {
    const r = runCondenserApproachEngine(s.m as unknown as Record<string, unknown>, { profile: condenserProfile } as unknown as Record<string, unknown>);
    const gaps:string[] = [];
...
const recipProfile = { compressor: { rla: 50 }, refrigeration: { refrigerantType: 'OTHER' } } as unknown as Record<string, unknown>;
...
for (const s of recipScenarios) {
  const r = runReciprocatingCompressorEngine(s.m as unknown as Record<string, unknown>, recipProfile as unknown as Record<string, unknown>);
  const gaps:string[] = [];
...
// Scroll compressor
const scrollProfile = { compressor: { rla: 10 }, refrigeration: { refrigerantType: 'R-410A' } } as unknown as Record<string, unknown>;
...
for (const s of scrollScenarios) {
  const r = runScrollCompressorEngine(s.m as unknown as Record<string, unknown>, scrollProfile as unknown as Record<string, unknown>);
  const gaps:string[] = [];
  if (r.flags.compressionStatus && r.flags.compressionStatus !== 'ok') {
...
// Reversing valve
const revProfile = { reversingValve: { solenoid: { voltage: 24 } } } as unknown as Record<string, unknown>;
const revScenarios = [
...
for (const s of revScenarios) {
  const r = runReversingValveEngine(s.m as unknown as Record<string, unknown>, revProfile as unknown as Record<string, unknown>);
  const gaps:string[] = [];
...
const out = { scannedAt: easternNow.toISOString(), findings };
