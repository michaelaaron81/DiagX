...
import { saveEntry, loadEntry, removeEntry, listEntries } from '../src/cli/localOverrides';
...
describe('localOverrides helpers', () => {
  ...
  it('save/load works', () => {
    const entry = { profileId: 'p1', pt: [[0, 10], [50, 200]], description: 'test', savedAt: new Date().toISOString() } as any;
    saveEntry(entry);
  ...
  it('list entries shows saved', () => {
    const e1 = { profileId: 'alpha', pt: [[10,100]], savedAt: new Date().toISOString() } as any;
    const e2 = { profileId: 'beta', pt: [[20,200]], savedAt: new Date().toISOString() } as any;
  ...
  it('remove entry works and returns false for missing', () => {
    const e1 = { profileId: 'z1', pt: [[10,100]], savedAt: new Date().toISOString() } as any;
    saveEntry(e1);
