import { DiagnosticModule, ValidationResult, Recommendation, MeasurementHelp, DiagnosisExplanation, ModuleMetadata, ModuleHelp, formatTemperature, formatFlow } from '../../shared/wshp.types';
...
export class AirsideDiagnosticModule implements DiagnosticModule<WaterCooledUnitProfile, AirsideMeasurements, AirsideEngineResult> {
  ...
  diagnose(measurements: AirsideMeasurements, profile: WaterCooledUnitProfile) {
    ...
    if (!validation.ok) {
      ...
      if (hasError) {
        const err: any = new Error('Airside measurement validation failed');
        err.validation = validation;
        throw err;
      }
    }

    return runAirsideEngine(measurements, profile as any);
  }
