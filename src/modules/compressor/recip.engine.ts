...
function analyzeCurrent(measured: number | undefined, rla: number | undefined): { status: DiagnosticStatus } {
...
function analyzeUnloading(total?: number, unloaded?: number): ReciprocatingUnloadingInfo | undefined {
  if (!total || total <= 0) return undefined;
  if (unloaded === undefined) return undefined;

  const status: DiagnosticStatus = unloaded < 0 || unloaded > total ? 'alert' : unloaded === 0 ? 'ok' : 'warning';
...
function deriveRecipHealthFlags(
  ratioStatus: DiagnosticStatus,
  sounds: ReciprocatingCompressorMeasurements['soundCharacteristics'],
): { reedValveSuspected?: boolean; pistonRingWearSuspected?: boolean } {
  const health: { reedValveSuspected?: boolean; pistonRingWearSuspected?: boolean } = {};
...
export function validateReciprocatingCompressorMeasurements(measurements: ReciprocatingCompressorMeasurements): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];

  if (measurements.suctionPressure === undefined || measurements.suctionPressure <= 0) errors.push('Suction pressure must be > 0 PSIG.');
  if (measurements.dischargePressure === undefined || measurements.dischargePressure <= 0) errors.push('Discharge pressure must be > 0 PSIG.');
  if (measurements.dischargePressure !== undefined && measurements.suctionPressure !== undefined && measurements.dischargePressure <= measurements.suctionPressure) errors.push('Discharge pressure must be higher than suction pressure.');
  if (measurements.suctionTemp < -40 || measurements.suctionTemp > 200) warnings.push('Suction temperature is outside typical range (-40 to 200Â°F).');
  if (measurements.dischargeTemp !== undefined) {
    if (measurements.dischargeTemp < measurements.suctionTemp) warnings.push('Discharge temperature should normally be higher than suction temperature.');
    if (measurements.dischargeTemp > 275) warnings.push('Discharge temperature is very high (>275Â°F) â€“ compressor stress risk.');
  }
  if (measurements.isRunning === false) warnings.push('Compressor marked as not running; readings may not represent operating conditions.');

  return { valid: errors.length === 0, errors: errors.length ? errors : undefined, warnings } as ValidationResult;
}
...
export function generateRecipRecommendations(params: {
  ...
  recipHealth?: { reedValveSuspected?: boolean; pistonRingWearSuspected?: boolean };
}): Recommendation[] {
...
export function runReciprocatingCompressorEngine(measurements: ReciprocatingCompressorMeasurements, profile: WaterCooledUnitProfile): ReciprocatingCompressorResult {
  ...
  const recommendations: Recommendation[] = generateRecipRecommendations({ compressionRatio: ratio, compressionStatus: comp.status, current: measurements.compressorCurrent, currentStatus: curr.status, compressorId: measurements.compressorId, rla, overallStatus, unloadingStatus: unloadingAnalysis?.status, recipHealth: health });
  ...
  const values: ReciprocatingCompressorValues = {
  ...
  const flags: ReciprocatingCompressorFlags = {
  ...
  const result: ReciprocatingCompressorResult = {
  ...
  } as ReciprocatingCompressorResult;
