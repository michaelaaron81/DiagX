// Utilities for validating user-supplied PT tables (pressure-temperature)
...
export function validatePTChart(pt: PTChartData | undefined | null): PTValidationResult {
  const errors: string[] = [];
  ...
  for (let i = 0; i < pt.length; i++) {
    ...
    if (lastP !== null) {
      if (p === lastP) warnings.push(`Row ${i} pressure ${p} duplicates previous row`);
      if (p < lastP) errors.push(`Row ${i} pressure ${p} is not ascending (previous ${lastP})`);
    }
    lastP = p;
  }
