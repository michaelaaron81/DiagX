...
export function validateReversingValveMeasurements(measurements: ReversingValveMeasurements, profile: WaterCooledUnitProfile): ValidationResult {
  ...
  if (!hasReversingValve(profile)) errors.push('This diagnostic module requires a heat pump with reversing valve');

  const ports = measurements.reversingValvePortTemps;
  if (!ports) errors.push('reversingValvePortTemps required');
  else {
    if (ports.dischargeInlet < 80 || ports.dischargeInlet > 300) warnings.push('Discharge inlet temp outside expected range (80-300°F)');
    if (ports.suctionReturn < 20 || ports.suctionReturn > 100) warnings.push('Suction return temp outside expected range (20-100°F)');
    if (ports.indoorCoilLine < 20 || ports.indoorCoilLine > 300) warnings.push('Indoor coil line temp outside reasonable range');
    if (ports.outdoorCoilLine < 20 || ports.outdoorCoilLine > 300) warnings.push('Outdoor coil line temp outside reasonable range');

    const temps = [ports.dischargeInlet, ports.suctionReturn, ports.indoorCoilLine, ports.outdoorCoilLine];
    const spread = Math.max(...temps) - Math.min(...temps);
    if (spread < 30) warnings.push('Temperature spread very small (<30°F) - valve may be stuck or unit not running');
  }
  ...
  if (measurements.solenoidVoltage !== undefined) {
    if (measurements.solenoidVoltage < 18 && measurements.solenoidVoltage > 0) warnings.push('Solenoid voltage low (<18V) - may not energize reliably');
    if (measurements.solenoidVoltage > 30) warnings.push('Solenoid voltage high (>30V) - verify reading');
  }

  return { valid: errors.length === 0, ok: errors.length === 0, errors: errors.length ? errors : undefined, warnings };
}

function getExpectedPattern(mode: 'cooling' | 'heating') {
  if (mode === 'cooling') return { hotPorts: ['discharge', 'outdoor'], coldPorts: ['indoor', 'suction'] };
  return { hotPorts: ['discharge', 'indoor'], coldPorts: ['outdoor', 'suction'] };
}

function analyzeSolenoid(actualVoltage: number, ratedVoltage: number) {
  if (actualVoltage < 2) return { status: 'no_voltage' as const };
  const minAcceptable = ratedVoltage * 0.85;
  if (actualVoltage < minAcceptable) return { status: 'low_voltage' as const };
...
function analyzeValvePattern(requestedMode: 'cooling' | 'heating', ports: any, hotPorts: string[], coldPorts: string[], tempSpread: number, compressionRatio: number) {
  if (tempSpread < 50) {
    return {
      ...
      status: 'critical' as DiagnosticStatus,
    };
  }
  ...
  if (matches) {
    if (compressionRatio < 3.0) {
      return { pattern: 'partial_leak' as const, status: 'alert' as DiagnosticStatus };
    }
    if (compressionRatio < 3.5) {
      return { pattern: 'partial_leak' as const, status: 'warning' as DiagnosticStatus };
    }
    return { pattern: 'correct' as const, status: 'ok' as DiagnosticStatus };
  }
  ...
  if (JSON.stringify(actualHot.slice().sort()) === JSON.stringify(opposite.hotPorts.slice().sort()) && JSON.stringify(actualCold.slice().sort()) === JSON.stringify(opposite.coldPorts.slice().sort())) {
    return { pattern: 'reversed' as const, status: 'alert' as DiagnosticStatus };
  }
...
export function runReversingValveEngine(measurements: ReversingValveMeasurements, profile: ReversingValveConfig): ReversingValveDiagnosis {
  const ports = measurements.reversingValvePortTemps;
  const temps = [ports.dischargeInlet, ports.suctionReturn, ports.indoorCoilLine, ports.outdoorCoilLine];
  ...
  const hotPorts: string[] = [];
  const coldPorts: string[] = [];
  if (ports.dischargeInlet >= hotThreshold) hotPorts.push('discharge'); else coldPorts.push('discharge');
  if (ports.suctionReturn >= hotThreshold) hotPorts.push('suction'); else coldPorts.push('suction');
  ...
  let overallStatus: DiagnosticStatus = patternAnalysis.status;
  if (solenoidStatus === 'no_voltage') overallStatus = 'alert';

  const recommendations = generateRecommendations(patternAnalysis, solenoidStatus, measurements, profile);

  const values: ReversingValveValues = {
  ...
  const flags: ReversingValveFlags = {
