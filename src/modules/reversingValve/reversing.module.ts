...
export const reversingValveMetadata: ModuleMetadata = {
...
export class ReversingValveDiagnosticModule implements DiagnosticModule<WaterCooledUnitProfile, ReversingValveMeasurements, ReversingValveDiagnosis> {
  metadata = reversingValveMetadata;
  help = reversingValveHelp;
  ...
  diagnose(measurements: ReversingValveMeasurements, profile: WaterCooledUnitProfile) {
    return runReversingValveEngine(measurements, profile);
  }

  getRecommendations(diagnosis: ReversingValveDiagnosis) {
  ...
  summarizeForReport(diagnosis: ReversingValveDiagnosis, profile: WaterCooledUnitProfile): string {
    ...
    if (diagnosis.solenoidStatus) lines.push(`Solenoid Status:      ${diagnosis.solenoidStatus.toUpperCase()}`);
    lines.push('');
    ...
    if (likely) lines.push(`  ${likely.rationale || likely.summary}`);
    return lines.join('\n');
  }
