...
function hasRelevantRecommendation(engineModule: string, result: Record<string, unknown>, keywords: string[]) {
  // result may contain `recommendations` but it's typed loosely in these scanner scripts
  const maybeRec = result.recommendations as unknown;
  if (!Array.isArray(maybeRec)) return false;
  return (maybeRec as unknown[]).some((r: Record<string, unknown>) => {
